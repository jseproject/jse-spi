...
import davaguine.jmac.util.Crc32;
import davaguine.jmac.util.APEException;
import davaguine.jmac.util.Prepare;
...
public class UnMAC {
    ...
    public void Initialize(IAPEDecompress pAPEDecompress) {
        ...
        if (pAPEDecompress == null) {
            Uninitialize();
            throw new APEException("Error Initializing UnMAC");
        }
    ...
    public int DecompressFrame(ByteArrayBuffer pOutputData, int FrameIndex) throws IOException {
    ...
    //functions
    private int DecompressFrameOld(ByteArrayBuffer pOutputData, int FrameIndex) throws IOException {
        ...
        if (nBlocks == 0)
            throw new APEException("Invalid Frame Index"); //nothing to do (file must be zero length) (have to return error)
        ...
        if ((m_pAPEDecompress.getApeInfoFormatFlags() & APEHeader.MAC_FORMAT_FLAG_CRC) <= 0) {
            ...
            if (nChecksum != nStoredCRC)
                throw new APEException("Invalid Checksum");
        } else {
