 ...
 * Time: 14:51:31
 */
// Naoko: implement java.io.Closeable to use try-with-resources mechanism
public abstract class IoFile implements Closeable, DataInput {

    public static IoFile createFile(final String name, final String mode) throws IOException {
        URL url = null;
        try {
            url = new URL(name);
            return new InputStreamIoFile(url);
        } catch (MalformedURLException e) {
            return new RandomAccessIoFile(new java.io.File(name), mode);
        }
    }
