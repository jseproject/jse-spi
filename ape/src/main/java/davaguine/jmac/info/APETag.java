...
public class APETag implements Comparator {
    ...
    public APETag(IoFile pIO) throws IOException {
        this(pIO, true);
    }

    public APETag(IoFile pIO, boolean bAnalyze) throws IOException {
    ...
    public APETag(String pFilename, boolean bAnalyze) throws IOException {
        m_spIO = new RandomAccessIoFile(new java.io.File(pFilename), "r");
        ...
        if (!m_bHasID3Tag) {
            ...
            if (m_footer != null && m_footer.GetIsValid(false)) {
                ...
                try {
                    ...
                    throw new APEException("Can't Read APE Tag Fields");
                }
            }
        }
    ...
    // Naoko: Add a "size" method for field enumeration
    public int numFields() {
        return m_aryFields.size();
    }
