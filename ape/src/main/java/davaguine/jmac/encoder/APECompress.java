...
public class APECompress extends IAPECompress {
    ...
    public void Start(String pOutputFilename, WaveFormat pwfeInput, int nMaxAudioBytes, int nCompressionLevel, byte[] pHeaderData, int nHeaderBytes) throws IOException {
        m_pioOutput = IoFile.createFile(pOutputFilename, "rw");
    ...
    public void StartEx(IoFile pioOutput, WaveFormat pwfeInput, int nMaxAudioBytes, int nCompressionLevel, byte[] pHeaderData, int nHeaderBytes) throws IOException {
    ...
    public void UnlockBuffer(int nBytesAdded, boolean bProcess) throws IOException {
        if (!m_bBufferLocked)
            throw new APEException("Error Undefined");
    ...
    private ByteArrayBuffer pBufferPointer = new ByteArrayBuffer();

    public ByteArrayBuffer LockBuffer(IntegerPointer pBytesAvailable) {
        if (m_pBuffer == null) {
            return null;
        }
    ...
    public void AddData(byte[] pData, int nBytes) throws IOException {
        ...
        while (nBytesDone < nBytes) {
            ...
            ByteArrayBuffer pBuffer = LockBuffer(m_nAddDataBytesAvailable);
    ...
    // Naoko: add for debug
    public int GetIdealFrameBytes() {
        return m_spAPECompressCreate.GetFullFrameBytes() - (m_nBufferTail - m_nBufferHead);
    }
    ...
    public int AddDataFromInputSource(InputSource pInputSource, int nMaxBytes) throws IOException {
        ...
        ByteArrayBuffer pBuffer = LockBuffer(m_nAddDataFromInputSourceBytesAvailavle);
    ...
    public void Kill() {
        if (m_pioOutput != null) {
            try {
                ...
                throw new APEException("Error while closing output stream", e);
            }
        }
        m_pioOutput = null;
    }
