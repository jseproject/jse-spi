...
public class APECompressCreate {
    ...
    public void InitializeFile(IoFile pIO, WaveFormat pwfeInput, int nMaxFrames, int nCompressionLevel, byte[] pHeaderData, int nHeaderBytes) throws IOException {
        // error check the parameters
        if (pIO == null || pwfeInput == null || nMaxFrames <= 0)
            throw new APEException("Bad Parameters");
    ...
    public void FinalizeFile(IoFile pIO, int nNumberOfFrames, int nFinalFrameBlocks, byte[] pTerminatingData, int nTerminatingBytes, int nWAVTerminatingBytes, int nPeakLevel) throws IOException {
    ...
    public void SetSeekByte(int nFrame, int nByteOffset) {
        if (nFrame >= m_nMaxFrames)
            throw new APEException("APE Compress Too Much Data");
        m_spSeekTable[nFrame] = nByteOffset;
    }

    public void Start(IoFile pioOutput, WaveFormat pwfeInput, int nMaxAudioBytes) throws IOException {
    ...
    public void Start(IoFile pioOutput, WaveFormat pwfeInput, int nMaxAudioBytes, int nCompressionLevel) throws IOException {
        Start(pioOutput, pwfeInput, nMaxAudioBytes, nCompressionLevel, null, IAPECompress.CREATE_WAV_HEADER_ON_DECOMPRESSION);
    }

    public void Start(IoFile pioOutput, WaveFormat pwfeInput, int nMaxAudioBytes, int nCompressionLevel, byte[] pHeaderData) throws IOException {
        Start(pioOutput, pwfeInput, nMaxAudioBytes, nCompressionLevel, pHeaderData, IAPECompress.CREATE_WAV_HEADER_ON_DECOMPRESSION);
    }

    public void Start(IoFile pioOutput, WaveFormat pwfeInput, int nMaxAudioBytes, int nCompressionLevel, byte[] pHeaderData, int nHeaderBytes) throws IOException {
        ...
        if ((pwfeInput.nChannels != 1) && (pwfeInput.nChannels != 2))
            throw new APEException("Input File Unsupported Channel Count");
        if ((pwfeInput.wBitsPerSample != 8) && (pwfeInput.wBitsPerSample != 16) && (pwfeInput.wBitsPerSample != 24))
            throw new APEException("Input File Unsupported Bit Depth");
