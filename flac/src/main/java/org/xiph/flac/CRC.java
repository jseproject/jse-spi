...
class CRC {
	...
	private static final char crc8_table[] = {// uint8 [256]
	 ...
	 * init = 0
	 */
	private static final char crc16_table[][] = {// uint16 [8][256] = {
...
/* #if 0
	static void crc16_init_table()
	{
		...
		for( int i = 0; i <= 0xFF; i++ ) {
			...
			for( int j = 0; j < 8; j++ ) {
				crc = (crc << 1) ^ ((0 != (crc & (1 << 15))) ? polynomial : 0);
			}

			crc16_table[0][i] = (char)crc;
		}

		for( int i = 0; i <= 0xFF; i++ ) {
			for( int j = 1; j < 8; j++ ) {
				crc16_table[j][i] = (char)(crc16_table[0][crc16_table[j - 1][i] >>> 8] ^ (crc16_table[j - 1][i] << 8));
			}
		}
	}
#endif */

	//static final int CRC16_UPDATE(final byte[] data, final int crc) {
	//	return (((crc << 8) & 0xffff) ^ crc16_table[0][((crc >>> 8) ^ data[ 0 ]) & 0xff]);
	//}
	/* this alternate may be faster on some systems/compilers */
// #if 0
	static final int CRC16_UPDATE(final int data, final int crc) {
		return (((crc << 8) ^ crc16_table[0][((crc >>> 8) ^ data) & 0xff]) & 0xffff);
	}
// #endif
	 ...
	 * init = 0
	 */
	static final int crc8(final byte[] data, final int len)
	{
		int crc = 0;

		for( int off = 0; off < len; off++ ) {
			crc = crc8_table[ (crc ^ data[ off ]) & 0xff ];
		}

		return crc;// uint8
	}

	static final int crc16(final byte[] data, int len)
	{
		int crc = 0;
		int off = 0;

		while( len >= 8 ) {
			crc ^= (data[off] << 8) | (data[off + 1] & 0xff);

			crc = crc16_table[7][(crc >>> 8) & 0xff] ^ crc16_table[6][crc & 0xFF] ^
			      crc16_table[5][data[off + 2] & 0xff] ^ crc16_table[4][data[off + 3] & 0xff] ^
			      crc16_table[3][data[off + 4] & 0xff] ^ crc16_table[2][data[off + 5] & 0xff] ^
			      crc16_table[1][data[off + 6] & 0xff] ^ crc16_table[0][data[off + 7] & 0xff];
		...
		while( len-- != 0 ) {
			crc = (crc << 8) ^ crc16_table[0][((crc >>> 8) ^ data[off++]) & 0xff];
		}

		return crc & 0xffff;// uint16
	}

	static final int crc16_update_words32(final int[] words, int off, int len, int crc)
	{
		while( len >= 2 ) {
			final int d0 = words[ off ];
			final int d1 = words[ off + 1 ];
			crc ^= d0 >> 16;

			crc = crc16_table[7][(crc >>> 8) & 0xff] ^ crc16_table[6][crc & 0xFF       ] ^
			      crc16_table[5][(d0 >>  8) & 0xFF ] ^ crc16_table[4][ d0        & 0xFF] ^
			      crc16_table[3][ d1 >>> 24        ] ^ crc16_table[2][(d1 >> 16) & 0xFF] ^
			      crc16_table[1][(d1 >>  8) & 0xFF ] ^ crc16_table[0][ d1        & 0xFF];
		...
		if( len != 0 ) {
			...
			crc = crc16_table[3][(crc >>> 8) & 0xff] ^ crc16_table[2][crc & 0xFF] ^
			      crc16_table[1][(d0 >> 8) & 0xFF  ] ^ crc16_table[0][d0  & 0xFF];
		}
	...
	/* java: 4 bytes per word, so it is not used
	static final char crc16_update_words64(final long[] words, final int len, char crc)
	{
		for( int i = 0; i < len; i++ ) {
			final long d = words[ i ];
			crc ^= d >>> 48;

			crc = (char)(crc16_table[7][crc >>> 8       ] ^ crc16_table[6][crc & 0xFF      ] ^
			      crc16_table[5][((int)(d >> 40)) & 0xFF] ^ crc16_table[4][((int)(d >> 32)) & 0xFF] ^
			      crc16_table[3][((int)(d >> 24)) & 0xFF] ^ crc16_table[2][((int)(d >> 16)) & 0xFF] ^
			      crc16_table[1][((int)(d >>  8)) & 0xFF] ^ crc16_table[0][((int)d)         & 0xFF]);
		}
