 ...
 * <pre>
 * byte data = header_base[header]
 * int data = ((int)header_base[header]) & 0xff
 * byte data = body_base[body]
 * int data = ((int)body_base[body]) & 0xff
 * </pre>
 */
...
*/
	...
	static void _ogg_crc_init() {
		int crc, polynomial = 0x04c11db7;// The same as the ethernet generator polynomial, although we use an unreflected alg and an init/final of 0, not 0xffffffff
	...
	*/
	...
	private static final int crc_lookup[][] = {// [8][256]
	...
	private static int _os_update_crc(int crc, final byte[] buffer, int off, int size) {
		while( size >= 8 ) {
			crc ^= ((int)buffer[ off ] << 24)
					| (((int)buffer[ off + 1 ] & 0xff) << 16)
		...
		while( size-- != 0 ) {
			crc = (crc << 8) ^ crc_lookup[ 0 ][ (crc >>> 24) ^ (buffer[ off++ ] & 0xff) ];
		}
		return crc;
	}
	...
	public final void checksum_set() {
		//if( og != null ) {
		    ...
		    crc_reg = _os_update_crc( crc_reg, this.header_base, this.header, this.header_len );
		    crc_reg = _os_update_crc( crc_reg, this.body_base, this.body, this.body_len );
