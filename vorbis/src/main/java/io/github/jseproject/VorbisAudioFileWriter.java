...
public class VorbisAudioFileWriter extends TAudioFileWriter {
    ...
    protected AudioOutputStream getAudioOutputStream(AudioFormat audioFormat, long lLengthInBytes, AudioFileFormat.Type fileType, TDataOutputStream dataOutputStream) throws IOException {
        return new VorbisAudioOutputParams(audioFormat, lLengthInBytes, (TNonSeekableDataOutputStream) dataOutputStream, ((VorbisFileFormatType) fileType).properties);
    }
    ...
    protected int writeImpl(AudioInputStream audioInputStream, AudioOutputStream audioOutputStream, boolean bNeedsConversion) throws IOException {
        ...
        vorbisInfo.init();
        int ret;
        if (vbr) ret = vorbisInfo.encode_init_vbr(channels, (int) outputFormat.getSampleRate(), quality < 0 ? 0.7f : quality);
        else ret = vorbisInfo.encode_init(channels, (int) outputFormat.getSampleRate(), -1, -1, -1);
        if (ret < 0) throw new IOException("Failed to initialize encoder");
        // add a comment
        vorbisComment.init();
    ...
    private static class VorbisAudioOutputParams implements AudioOutputStream {
        ...
        public VorbisAudioOutputParams(AudioFormat format, long length, TNonSeekableDataOutputStream out, Map<String, Object> properties) {
            this.format = format;
            this.length = length;
            this.out = out;
