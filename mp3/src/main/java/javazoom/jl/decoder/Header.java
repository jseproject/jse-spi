...
public final class Header
{
	...
	void read_header(Bitstream stream, CRC16[] crcp) throws BitstreamException
	{
		...
		do
		{
			...
			if (syncmode == Bitstream.INITIAL_SYNC)
			{
				...
				if (((headerstring >>> 20) & 1) == 0) // SZD: MPEG2.5 detection
					if (h_version == MPEG2_LSF)
						...
						throw stream.newBitstreamException(BitstreamError.UNKNOWN_ERROR);
			...
			if ((framesize >=0) && (framesizeloaded != framesize))
			{
				...
				throw stream.newBitstreamException(BitstreamError.INVALIDFRAME);
			}
	...
	/**
	 * Return VBR TOC.
	 * @return vbr toc or null if not available
	 */
	 ...
	 */
	public double ms_per_frame() // Naoko: changed to double
	{
		double tpf = h_vbr_time_per_frame[layer()] / frequency();
		// Naoko: now treat MPEG2_L3/MPEG25_L3 as MPEG1_L3
		// IDK right or wrong X_X
		// Reference: http://mpgedit.org/mpgedit/mpeg_format/mpeghdr.htm
		//if (h_layer == 3 && (h_version == Header.MPEG2_LSF || h_version == Header.MPEG25_LSF)) tpf /= 2;
		return tpf * 1000;
	}
	 ...
	 * @author Naoko Mitsurugi
	 */
	public double total_ms(int streamsize) // Naoko: changed to double
	{
		return max_number_of_frames(streamsize) * ms_per_frame();
	}
